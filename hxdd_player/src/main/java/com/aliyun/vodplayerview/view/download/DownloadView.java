...
public class DownloadView extends FrameLayout implements OnClickListener, CompoundButton.OnCheckedChangeListener {
    ...
    private void initColor() {
        ...
        tv_edit.setTextColor(StartPlayerUtils.getColorPrimary());

        TextView tv_delete_all = findViewById(R.id.tv_delete_all);
        tv_delete_all.setTextColor(StartPlayerUtils.getColorPrimary());

        ivDownloadDelete.setColorFilter(StartPlayerUtils.getColorPrimary());
        ivCloseEdit.setColorFilter(StartPlayerUtils.getColorPrimary());
//        ColorStateList colorStateList
        cbAllDownloadCheck.setButtonTintList(ColorStateList.valueOf(StartPlayerUtils.getColorPrimary()));
//        cbAllDownloadCheck.setBackgroundColor(StartPlayerUtils.getColorPrimary());
    }
...
//            addSection(tag, title, alivcDownloadingMediaInfos);
    ...
    public void initDownloadList(List<ChapterBean> chapterList) {
        String tag = DOWNLOADING_TAG;
        String title = getResources().getString(R.string.download_caching);
        ArrayList<AlivcDownloadMediaInfo> list = new ArrayList<>();
...
//        },350);
            ...
            section.setOnSectionItemClickListener(new OnSectionItemClickListener() {
                ...
                public void onItemClick(int posion, String tag) {
                    ...
                    if (tag.equals(DOWNLOADING_TAG)) {
                        ...
                        if (aliyunDownloadMediaInfo.getStatus() == AliyunDownloadMediaInfo.Status.NoDownload) {
                            if (onDownloadViewListener != null) {
                                onDownloadViewListener.onNewDownLoad(aliyunDownloadMediaInfo);
                                sectionAdapter.notifyItemChangedInSection(tag, positionInSection);
                            }
    ...
    public boolean hasAdded(AliyunDownloadMediaInfo info) {
        for (AlivcDownloadMediaInfo downloadMediaInfo : alivcDownloadingMediaInfos) {
            if (info.getNewPlayerId().equals(downloadMediaInfo.getAliyunDownloadMediaInfo().getNewPlayerId())) {
                if (downloadMediaInfo.getAliyunDownloadMediaInfo().getStatus() == AliyunDownloadMediaInfo.Status.NoDownload
                        || downloadMediaInfo.getAliyunDownloadMediaInfo().getStatus() == AliyunDownloadMediaInfo.Status.Prepare) {
                    return false;
                }
//                Context context = this.context.get();
//                if (context != null) {
...
//                }
    ...
    public void updateInfo(AliyunDownloadMediaInfo aliyunDownloadMediaInfo) {
        ...
        for (AlivcDownloadMediaInfo info : alivcDownloadingMediaInfos) {
            if (info.getAliyunDownloadMediaInfo().getNewPlayerId().equals(aliyunDownloadMediaInfo.getNewPlayerId())) {
                tmpInfo = info;
//                aliyunDownloadMediaInfo.setTitle(info.getAliyunDownloadMediaInfo().getTitle());
                info.setAliyunDownloadMediaInfo(aliyunDownloadMediaInfo);
                break;
            }
        }
...
//            removeDownloadingMeiaInfo(aliyunDownloadMediaInfo);
    ...
    public interface OnDownloadViewListener {
        ...
        void onNewDownLoad(AliyunDownloadMediaInfo downloadMediaInfo);
